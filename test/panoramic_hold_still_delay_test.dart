...
void main() {
  ...
  Map<String, dynamic> ocrFrame({required bool readable}) => {
        'type': 'ocr',
        'readable': readable,
      };
  ...
  test(
    ...
    () async {
      ...
      // bộ phận + OCR đọc được biển ngay. Đây là kịch bản khiến bug xuất hiện:
  ...
  test(
    ...
    () async {
      ...
      expect(controller.message?.message,
          anyOf(StringSheet.holdStillGuide, StringSheet.plateValidCaptured));
    },
    timeout: const Timeout(Duration(seconds: 60)),
  );
